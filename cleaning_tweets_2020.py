...
path='D:/PROJET PYTHON/'
#path = "/Users/samuel_br/Documents/Master TIDE/Python/Projet/Data"
...
def getMaxFile():
    for i,name in enumerate(filenames):
        if os.path.isfile(path+'2020tweets/'+name):
            print(name+ " exists")
        else:
...
####states, countries=get_US_State(election_2020_tweets_temp)
...
####election_2020_tweets_temp=pd.read_csv("C:/Users/Administrateur/Desktop/PROJET PYTHON/tweets/tweets_12.csv",sep=";")
####places_country,places_state=getPlaceCountry(election_2020_tweets_temp)
...
####sources_t=getSourceType(election_2020_tweets_temp)
...
####texts, types=getRealText(election_2020_tweets_temp)         

#Ajouter des colonnes : "mentions_dem", "mentions_rep", "contains_link", booleans ou str des termes en question
dem_keywords=["Joseph", "Joe", "Biden", "Kamala", "Harris","Democrats", "Democrat","Democratic","DNC"]
rep_keywords=["Trump","Donald","Mike", "Michael", "Pence","Republicans","Republican","RNC","GOP"]

#Utiliser les hastags pour donner une première classification / CV pour le NLP
pro_dem_hastags_keywords=["#resisters","#resist","#StrongerWithBiden", "#trumpout","#GetDownTrump","#yourefired","#CountEveryLegalVote", "#byedon", "#ByeDon2020", "#notmypresident", "#BlackLivesMatter","#BLM","#dumptrump","#LastTimeTrumpPaidTaxes","#NeverTrump","#strongertogether"]
pro_rep_hastags_keywords=["#stopthecount", "#KeepAmericaGreat", "#MakeAmericaGreatAgain","#MAGA","#MAGA2020","#AmericaFirst","#RiggedElection", "#iSeeDeadPeopleVote", "#TheWalkingVoters" ]
...
####m_dem,m_rep,h_dem,h_rep=getPolMentions(election_2020_tweets_temp)            
...
####isprodem=election_2020_tweets_temp.loc[:,'hastags_pro_dem'].apply(IsPro)
####isprorep=election_2020_tweets_temp.loc[:,'hastags_pro_rep'].apply(IsPro)
...
####states,country=locate(election_2020_tweets)

            
########
# election_2020_tweets_temp["real_text"]=texts
# election_2020_tweets_temp["hastags_pro_dem"]=h_dem
# election_2020_tweets_temp["hastags_pro_rep"]=h_rep
# election_2020_tweets_temp["loc_country"]=countries
# election_2020_tweets_temp["loc_state"]=states
# election_2020_tweets_temp["place_country"]=place_country
# election_2020_tweets_temp["place_state"]=place_state
# election_2020_tweets_temp["mentions_dem"]=m_dem
# election_2020_tweets_temp["mentions_rep"]=m_rep
# election_2020_tweets_temp["pro_dem"]=isprodem
# election_2020_tweets_temp["pro_rep"]=isprorep
# election_2020_tweets_temp["source_type"]=sources_t
# election_2020_tweets_temp["tweet_type"]=types


# tweets=election_2020_tweets_temp.loc[:,["tweet_type",
#                                         "id",
#                                         "created_at",
#                                         "loc_country",
#                                         "loc_state",
...
def cleanUp(nfile):
    election_2020_tweets_temp=pd.read_csv(path+"2020tweets/tweets_"+str(nfile)+".csv",sep=";")
    ...
    place_country,place_state=getPlaceCountry(election_2020_tweets_temp)
    election_2020_tweets_temp["place_country"]=place_country
    election_2020_tweets_temp["place_state"]=place_state
    
    states,countries=locate(election_2020_tweets_temp)

    election_2020_tweets_temp["state"]=states
    election_2020_tweets_temp["country"]=countries
    ...
    #Export
    tweets.to_csv(path+"2020cleantweets/cleantweets_"+str(nfile)+".csv",sep=";",index=False)
    return("Success, tweets saved to cleantweets_"+str(nfile)+".csv")

for i in range(562,592):
    cleanUp(i)
